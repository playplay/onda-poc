...
async def analyze_post(db: AsyncSession, post: Post) -> GeminiAnalysis | None:
    """Run Gemini analysis on a single post via Vercel AI Gateway."""
    # OIDC token is required for Vercel AI Gateway auth
    ...
    try:
        ...
        print(f"Gemini analysis failed for post {post.id}: {e}")
        return None
